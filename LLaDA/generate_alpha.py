...
def generate(
    ...
    injection_step=None,
    # === NEW ===
    alpha0: float = 0.3,  # Adaptive/Adaptive-step 的初始权重
):
    """
    Args:
        ...
        remasking: Remasking strategy. 'low_confidence' | 'random' | 'rate' | 'adaptive' | 'adaptive_step'.  # === NEW ===
        mask_id: The toke id of [MASK] is 126336.
        random_rate: For 'rate' mode only (kept for backward compatibility).
        injection_step: jailbreak token injection step (optional).
        alpha0: initial randomness weight for 'adaptive' / 'adaptive_step'.  # === NEW ===
    """
    ...
    steps = steps // num_blocks  # 注意：下面的 i∈[0, steps-1] 是 block 内部步数
    ...
    for num_block in range(num_blocks):
        ...
        for i in range(steps):
            ...
            # 仅允许本 block 范围内的 token 被更新（阻止越界位置参与这一步的选择）
